...
public class OrGateTest {
	...
	@Test
	public void test() {
		//(deco1 == value1) v (deco2 != value2)
		//Fall (f v f) = f
		insertValue(valuefalse, value2);
		assertTrue("Fall (f v f) = f stimmt nicht", !c.condition(((Condition)bXML).getRule()));
		
		//Fall (f v t) = f
		insertValue(valuefalse, valuefalse);
		assertTrue("Fall (f v t) = t stimmt nicht", c.condition(((Condition)bXML).getRule()));
		
		//Fall (t v f) = f
		insertValue(value1, value2);
		assertTrue("Fall (t v f) = t stimmt nicht", c.condition(((Condition)bXML).getRule()));
